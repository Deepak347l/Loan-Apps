...
import com.deepakdevloper.loanapp.databinding.ActivityPocketdetailsBinding
...
class Pocketdetails : AppCompatActivity() {
    private lateinit var binding: ActivityPocketdetailsBinding
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        binding = ActivityPocketdetailsBinding.inflate(layoutInflater)
    ...
    private fun saveUserData() {
        ...
        hashMap.put("Job Name",binding.num.text.toString())
        ...
        FirebaseDatabase.getInstance().getReference("POCKET_LOAN").child(FirebaseAuth.getInstance().currentUser?.uid.toString())
            .child("POCKET_DETAILS").child(FirebaseAuth.getInstance().currentUser?.uid.toString()).updateChildren(hashMap)
    }
    ...
    override fun onResume() {
        ...
        try{
            FirebaseDatabase.getInstance().getReference("POCKET_LOAN")
                ...
                .child("POCKET_DETAILS").child(FirebaseAuth.getInstance().currentUser?.uid.toString()).addValueEventListener(object:
                    ValueEventListener {
                    override fun onDataChange(snapshot: DataSnapshot) {
                        try{
                            if (snapshot.exists()){
                                if (snapshot.child("child").value == true){
                                    val i = Intent(this@Pocketdetails,FaseVarification::class.java)
                                    i.putExtra("pocket","POCKET_DETAILS")
                                    startActivity(i)
                                    finish()
                                }
                            ...
                            Toast.makeText(this@Pocketdetails,e.message.toString(), Toast.LENGTH_SHORT).show()
                        }
                    }

                    override fun onCancelled(error: DatabaseError)
                    {
                        Toast.makeText(this@Pocketdetails, error.message, Toast.LENGTH_SHORT).show()
                    }
